    ...
    function Scroller(elem, settings) {
        ...
        */
        that.show = function (prevAnim) {
            ...
            var html = '<div role="dialog" class="'
                + s.theme + ' dw-' + s.display
                + (prefix ? ' dw' + prefix : '') + '">'
                + (s.display == 'inline' ? '<div class="dw dwbg dwi"><div class="dwwr">' : '<div class="dw-persp"><div class="dwo"></div><div class="dw dwbg ' + mAnim + '"><div class="dw-arrw"><div class="dw-arrw-i"><div class="dw-arr"></div></div></div><div class="dwwr"><div aria-live="assertive" class="dwv'
                + (s.headerText ? '' : ' dw-hidden') + '"></div>')
                + '<div class="dwcc">';

            /*
             * 修改代码使其只显示小时 ，如果是zh即中文显示24小时时间，则默认是时：分
             * s.wheels=[[{"label":"时","value":[00,01,...],"key":[]},{"label":"分",..}]]
             * 通过wheels[0].pop删除代表分的对象
             */
            s.wheels[0].pop();
            $.each(s.wheels, function (i, wg) { // Wheel groups
                //wheel格式 [[{},{},{}]]
                //s.wheels表示数组，每个值是一个对象
                console.log(s.wheels);   //wg是对象，如过preset是time则代表“时|分”  如果是date则代表“日|月|年”
                html += '<div class="dwc' + (s.mode != 'scroller' ? ' dwpm' : ' dwsc') + (s.showLabel ? '' : ' dwhl') + '"><div class="dwwc dwrc"><table cellpadding="0" cellspacing="0"><tr>';
                $.each(wg, function (j, w) { // Wheels
                    wheels[l] = w;
                    lbl = w.label !== undefined ? w.label : j;
                    html += '<td><div class="dwwl dwrc dwwl' + l + '">'
                        + (s.mode != 'scroller' ? '<div class="dwb-e dwwb dwwbp" style="height:'
                        + hi + 'px;line-height:' + hi
                        + 'px;"><span>+</span></div><div class="dwb-e dwwb dwwbm" style="height:'
                        + hi + 'px;line-height:' + hi
                        + 'px;"><span>&ndash;</span></div>' : '') + '<div class="dwl">'
                        + lbl + '</div><div tabindex="0" aria-live="off" aria-label="' + lbl
                        + '" role="listbox" class="dwww"><div class="dww" style="height:'
                        + (s.rows * hi) + 'px;min-width:' + s.width
                        + 'px;"><div class="dw-ul">';
                    // Create wheel values
                ...
                //修改为显示小时数和“时”
                html += '<td><div class="dw-i" style="color:#fff;background:#4c4c4c;height:70px;line-height:70px;width:70px;font-size:24px;">时</div></td></tr></table></div></div>';
