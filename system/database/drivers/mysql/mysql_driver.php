...
class CI_DB_mysql_driver extends CI_DB {
    ...
    /**
     * ON DUPLICATE UPDATE BATCH statement
     *
     ...
     * @return    string
     */
    public function _duplicate_insert_batch($table, $data)
    {
        $keystr = array();
        $value_str = array();
        foreach ($data as $values) {
            $keystr = array_keys($values);
            $valstr = array();
            foreach($values as $val) {
                $valstr[] = $val;
            }
            $value_str[] = "(".implode(', ',$valstr).")";
        }

        $updatestr = array();
        foreach ($keystr as $key) {
            $updatestr[] = "{$key}=values({$key})";
        }

        $sql  = "INSERT INTO ".$table." (".implode(', ',$keystr).") ";
        $sql .= "VALUES ".implode(', ', $value_str)." ";
