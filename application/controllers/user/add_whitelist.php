 ...
 *   
*/
class Add_whitelist extends CI_Controller {
    ...
    public function index()
    {
        ...
        //获取已有数量
        $curr_count = $this->user_service->white_count($this->sem_user);
        if($curr_count + count($this->white_list) > 5)
        {
            $res['error_code'] = '9';
            $res['error_msg'] = "domain greater than 5";
            $this->output->set_output(json_encode($res));
            return FALSE;
        }
        //添加
        $add_res = $this->user_service->add_whitelist($this->sem_user,$this->white_list);
        if(!$add_res)
        {
            $res['error_code'] = '10';
...
/* End of file add_whitelist.php */
/* Location: ./application/controllers/user/add_whitelist.php */
